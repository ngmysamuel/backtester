...
def get_max_drawdown(df: pd.DataFrame):
    ...
    return (max_drawdown, max_drawdown_date.strftime("%d %b, %Y"), longest_streak, longest_start.strftime("%d %b, %Y"), longest_end.strftime("%d %b, %Y"))
...
def rolling_sharpe(df: pd.DataFrame, interval: str, window: str) -> plotly.graph_objs.Figure:
    match window:
        ...
        case _:
            window = 126
    annualization_factor = get_annualization_factor(interval)
    rolling_sharpe = df["returns"].rolling(window).apply(lambda x: np.sqrt(annualization_factor) * (np.mean(x) / np.std(x, ddof=1)) if np.std(x, ddof=1) != 0 else 0.0)
...
def find_top_drawdowns(df: pd.DataFrame, n: int = 5) -> pd.DataFrame:
    ...
    for date, row in df.iterrows():
        if not in_drawdown and row["underwater"]:
            in_drawdown = True
            current_period = {"Peak Date": (df["equity_curve"].loc[:date] == row["hwm"]).idxmax(), "Peak Value": row["hwm"], "Trough Date": date, "Trough Value": row["equity_curve"], "Max Drawdown %": row["drawdown_percent"]}
        elif in_drawdown:
            ...
            while outstanding_quantity > 0:
                if shorts[ticker]:  # there exists someting in the SHORT deque for that ticker - COVER SHORT
                    ...
                    closed_trades.append(
                        {
                            "ticker": ticker,
                            "entry_date": earliest_short["date"],
                            "exit_date": trade_date,
                            "type": "BUY",
                            "quantity": traded_quantity,
                            "nett position": net_positions[ticker],
                            "entry_price": earliest_short["price"],
                    ...
                    longs[ticker].append({"price": unit_price, "date": trade_date, "quantity": outstanding_quantity})
                    outstanding_quantity = 0
            ...
            while outstanding_quantity > 0:
                if longs[ticker]:  # there exists someting in the LONG deque for that ticker - SELL
                    ...
                    closed_trades.append(
                        {
                            "ticker": ticker,
                            "entry_date": earliest_long["date"],
                            "exit_date": trade_date,
                            "type": "SELL",
                            "quantity": traded_quantity,
                            "nett position": net_positions[ticker],
                            "entry_price": earliest_long["price"],
                            "exit_price": unit_price,
                            "pnl": pnl,
                            "return_pct": return_pct,
                        }
                    ...
                    shorts[ticker].append({"price": unit_price, "date": trade_date, "quantity": outstanding_quantity})
                    outstanding_quantity = 0
    if closed_trades:
...
def plot_equity_curve_with_trades(ticker: str, df_trades: pd.DataFrame, df_equity: pd.DataFrame):
    ...
    fig.update_layout(title_text="Equity Curve with Trades", xaxis_title="Date", yaxis_title="Equity", legend_title="Trade Direction")
...
def plot_stacked_pnl_by_holding_period(ticker: str, df: pd.DataFrame):
    ...
    fig.add_trace(go.Bar(x=summary.index, y=summary["Gross Profit"], name="Gross Profit", marker_color="#2ca02c", hovertemplate="<b>%{x}</b><br>Gross Profit: $%{y:,.2f}<extra></extra>"))

    # Add Gross Loss bars (red)
    fig.add_trace(go.Bar(x=summary.index, y=summary["Gross Loss"], name="Gross Loss", marker_color="#d62728", hovertemplate="<b>%{x}</b><br>Gross Loss: $%{y:,.2f}<extra></extra>"))

...
st.set_page_config(layout="wide", page_title="Risk Analysis")
...
with col_var3:
    st.segmented_control("Method", options=["Historical", "Parametric"], selection_mode="single", key="var_method")


##### Drawdown #####
# Equity Graph
fig_equity = go.Figure()
fig_equity.add_trace(go.Scatter(x=drawdown_df.index, y=drawdown_df["equity_curve"], mode="markers+lines", line=dict(color="rgba(3, 52, 110, 1.0)", width=2), marker=dict(color="rgba(135, 206, 250, 0.0)")))
fig_equity.update_traces(hovertemplate="<b>Date</b>: %{x|%Y-%m-%d}<br><b>Equity</b>: %{y:.2f}x<br><extra></extra>")
...
fig_underwater.update_traces(hovertemplate="<b>Date</b>: %{x|%Y-%m-%d}<br><b>Drawdown</b>: %{y:.2f}%<br><b>Days Underwater</b>: %{customdata[0]}<extra></extra>", customdata=drawdown_df[["days_underwater"]])
...
fig_drawdown.update_traces(hovertemplate="<b>Date</b>: %{x|%Y-%m-%d}<br><b>Drawdown</b>: %{y:.2f}%<br><b>Max Drawdown</b>: %{customdata[0]}<extra></extra>", customdata=drawdown_df[["max_drawdown"]])
...
with col2:
    ...
    for i, row in top_drawdowns.head(1).iterrows():  # Annotate only the worst one to keep it clean
        fig_drawdown.add_annotation(x=row["Trough Date"], y=float(row["Max Drawdown %"].strip("%")), text=f"Worst DD: {row['Max Drawdown %']}", showarrow=True, arrowhead=2, arrowcolor="red", ax=-40, ay=-40, bgcolor="white")

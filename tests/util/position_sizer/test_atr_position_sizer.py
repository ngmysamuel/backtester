...
# Mocking BarTuple locally to avoid importing the whole util module if dependencies are complex
# Assuming BarTuple has high, low, close. Added Index for completeness.
BarTuple = namedtuple('BarTuple', ['high', 'low', 'close', 'Index'])
...
class TestATRPositionSizer:
    ...
    def _create_bars(self, prices):
        ...
        for i, (h, l, c) in enumerate(prices):
            bars.append(BarTuple(high=h, low=l, close=c, Index=i))
        return bars
    ...
    def test_initial_calculation_arithmetic_mean(self, sizer):
        """
    ...
    def test_subsequent_calculation_wilders(self, sizer):
        """
        ...
        prev_bar = BarTuple(high=10, low=10, close=10, Index=0)
        # Current Bar (Index -1 in history). 
        # H=14, L=10, PrevC=10. TR = max(4, 4, 0) = 4.
        curr_bar = BarTuple(high=14, low=10, close=12, Index=1)
    ...
    def test_get_position_size_integer_rounding(self, sizer):
    ...
    def test_get_position_size_decimal_rounding(self, sizer):
    ...
    def test_get_position_size_no_atr_data(self, sizer):
    ...
    def test_get_position_size_zero_atr_guard(self, sizer):
        """Should return None if ATR is 0 to prevent ZeroDivisionError."""
        sizer.historical_atr["AAPL"] = [0.0]
        size = sizer.get_position_size(0.01, 10000, 0, "AAPL")
        assert size is None

    def test_get_position_size_reuse_previous(self, sizer):
